...
public class PlayerControl : MonoBehaviour
{
	...
	[SerializeField] private AudioClip[] jumpClips;			// Array of clips for when the player jumps.
	[SerializeField] private float jumpForce = 1000f;			// Amount of force added when the player jumps.
	[SerializeField] private AudioClip[] taunts;				// Array of clips for when the player taunts.
	[SerializeField] private float tauntProbability = 50f;	// Chance of a taunt happening.
	[SerializeField] private float tauntDelay = 1f;			// Delay for when the taunt should happen.
	...
	void FixedUpdate ()
	{
		...
		if(_horizontal * _rb.velocity.x < maxSpeed)
			...
			_rb.AddForce(Vector2.right * _horizontal * moveForce);
